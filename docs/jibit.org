...
** Nikon lens EXIF...                                                :nikon:

- NEF: exiftool reads LensID okay
- Darktable: parses and shows the LensID okay
- Darktable-exported jpeg: no...

Here's the json outputs from exiftool, highlighting the difference. We
can see that "Lens" is adequate and "LensSpec" great. But LensSpec is
not available for Leica.

When we use LensID as primary and LensSpec and Lens as backups, we're
getting different lens data depending on what raw processor was used.
...
* TODO Make "user-select none" as default behavior in every comp?

The selectively allow user select where it makes sense.
...
* DONE Raw indicator
CLOSED: [2020-07-12 Sun 12:18]
:LOGBOOK:
- State "DONE"       from "TODO"       [2020-07-12 Sun 12:18]
:END:

Ditch the "RAW" item and maybe show film perforations.
...
* DONE Export options
CLOSED: [2021-04-10 Sat 15:51]
:LOGBOOK:
- State "DONE"       from "TODO"       [2021-04-10 Sat 15:51]
:END:

- Export as $preset
- Export full (as JPEG)

  Meaning we convert to JPEG if it's not JPEG. Doesn't apply to raws.
  
- Automatic tag when a photo is exported?
  - We can just also make mass operations on them. Like select 15
    photos, tag "flicker" and export.

** DONE Export filenames into something better
CLOSED: [2020-07-14 Tue 17:54]
:LOGBOOK:
- State "DONE"       from "TODO"       [2020-07-14 Tue 17:54]
:END:

We're giving them easily sortable names based on TAKEN_TS. On name
conflicts we throw letters at the end of the name.
* TODO Record dates of image export

- Then we can have things like filters: "exported on:" 

* DONE Fancydate improvements
CLOSED: [2020-07-13 Mon 20:18] SCHEDULED: <2020-07-13 Mon>
:LOGBOOK:
- State "DONE"       from "TODO"       [2020-07-13 Mon 20:18]
:END:

- when custom range selected, make sure the begin...end dates are
  always sensical
  - ie. reset the other end if they cross over
- we use =data-bound-input= and now there's a tiny bit more to
  refactor.

* DONE Refactor fancydate into its own ns
CLOSED: [2020-07-15 Wed 19:41] SCHEDULED: <2020-07-15 Wed>
:LOGBOOK:
- State "DONE"       from "TODO"       [2020-07-15 Wed 19:41]
:END:

Let's start splitting this beast up from here.

* DONE Selections sluggish due to tags?
CLOSED: [2020-06-27 Sat 18:58]
:LOGBOOK:
- State "DONE"       from "TODO"       [2020-06-27 Sat 18:58]
:END:
[2020-06-27 Sat]

- On our table of 800 slides, having tags imposes ~1.4 s delays on
  selections!
- Removing tags from showing on slides, the delays reduce to ~30-40 ms
- This is just a selection so there shouldn't be a big difference.
- culprit is the ~tag-view~ component...
  - ah-ha! Of course. There's a subscription to selection. That
    explains it all.
  - selection changes, it causes re-renders.
- implemented (copypasted) a tiny variant of the tag which doesn't do
  the usability things... only represents the tag
- this new variant helped a great deal. Selections are instant again.
  But taggings can be sluggish.

* TODO Tagging on huge table is sluggish

- 800 slides on the table, tagging takes +400 ms
- Too much.
...
* TODO [#B] When tag is deleted, phantom little divs remain
...
** DONE Raw indication: show another div over the thumbnail
CLOSED: [2020-07-12 Sun 12:18]
:LOGBOOK:
- State "DONE"       from "TODO"       [2020-07-12 Sun 12:18]
:END:
...
Browsers and browser extensions like Tridactyl, Vimperator and Vimium
take over many of the keys.

Vanilla browsers don't, probably, because we =.preventDefault= on key
action.

But even still, extensions or no, I wonder if it's a good idea to take
over some customary browser shortcuts at all.

Instead of C-a selecting all slides, M-a works across the test bench.
Let's go with Alt.
...
Emacs-style global keybinds. Let's see how it works.

Holy shit it works perfectly out of the box.
** NOTSURE re-pressed
CLOSED: [2021-01-01 Fri 17:51]
...
* TODO Statistics for gamification purposes

- Show Stats about "count and % of pictures tagged/titled"
- Show histograms about the counts, examples:
  - Untagged content by the year
